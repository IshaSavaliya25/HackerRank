...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    ...
    char** alloc_grid = malloc(r * sizeof(char*));
    for (int i = 0; i < r; i++) {
        alloc_grid[i] = malloc((c + 1) * sizeof(char));
        for (int j = 0; j < c; j++) alloc_grid[i][j] = 'O';
        alloc_grid[i][c] = '\0';
    }
    ...
    char** detonate(char** g) {
        char** res = malloc(r * sizeof(char*));
        for (int i = 0; i < r; i++) {
            res[i] = malloc((c + 1) * sizeof(char));
            for (int j = 0; j < c; j++) res[i][j] = 'O';
            res[i][c] = '\0';
        }
    ...
    char** first = detonate(grid);

    if (n % 4 == 3) {
        *result_count = r;

        for (int i = 0; i < r; i++) free(alloc_grid[i]);
        free(alloc_grid);
        return first;
    } else {
